...
==================================================
    ...
    def get_vehicles(self, filter_brand=None, filter_model=None, filter_payment=None, sort_by=None):
        try:
            vehicles_list = []
            ...
            if sort_by:
                if sort_by.lower() == "рік":
                    filtered_vehicles.sort(key=lambda v: v["year"], reverse=True)
                elif sort_by.lower() == "власник":
                    filtered_vehicles.sort(key=lambda v: v["owner"])
                elif sort_by.lower() == "вартість":
                    filtered_vehicles.sort(key=lambda v: v["total_cost"], reverse=True)
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            print("Фільтрація та сортування (залиште порожнім для відображення всіх)")
            filter_brand = input("Фільтр за маркою: ") or None
            filter_model = input("Фільтр за моделлю: ") or None

            payment_options = "\n1 - Оплачено\n2 - Не оплачено\n0 - Всі"
            filter_payment_choice = input(f"Фільтр за оплатою {payment_options}: ")
            filter_payment = None
            ...
            sort_options = "\n1 - За роком випуску\n2 - За власником\n3 - За вартістю робіт\n0 - Без сортування"
            sort_choice = input(f"Сортування {sort_options}: ")
            sort_by = None
            if sort_choice == "1":
                sort_by = "рік"
            elif sort_choice == "2":
            ...
            vehicles = system.get_vehicles(filter_brand, filter_model, filter_payment, sort_by)

            if not vehicles:
                print("Немає транспортних засобів, що відповідають критеріям.")
            else:
                print(tabulate([[v['id'][:8], v['brand'], v['model'], v['year'], v['reg_number'], v['owner'],
                                 v['orders_count'], v['unpaid_orders']]
                                for v in vehicles],
                               headers=["ID", "Марка", "Модель", "Рік", "Реєстр. номер", "Власник", "Заявок",
                                        "Неоплачено"]))
